...
export const complaints = pgTable(
  "complaints",
  {
    ...
    // Officer oversight & contact details
    officerName: text("officer_name").notNull().default("Shri R. K. Sharma, IAS"),
    officerDesignation: text("officer_designation").notNull().default("Director & Nodal Grievance Redressal Officer"),
    officerEmail: text("officer_email").notNull().default("nodal.grievance@gov.in"),
    officerPhone: text("officer_phone").notNull().default("+91-11-2338-4000"),
    officerOffice: text("officer_office").notNull().default("Room 304, Shram Shakti Bhawan, Rafi Marg, New Delhi - 110001"),
    // Appellate authority oversight
    appellateOfficerName: text("appellate_officer_name").notNull().default("Smt. Sunita Verma, IAS"),
    appellateOfficerDesignation: text("appellate_officer_designation").notNull().default("Joint Secretary & First Appellate Authority"),
    appellateOfficerEmail: text("appellate_officer_email").notNull().default("appellate.authority@gov.in"),
    appellateOfficerPhone: text("appellate_officer_phone").notNull().default("+91-11-2338-9900"),
    // SLA & Accountability fields
    slaDeadline: text("sla_deadline").notNull().default(""),
    slaDays: integer("sla_days").notNull().default(30),
    publicExplanation: text("public_explanation").notNull().default(""),
...
export const appeals = pgTable(
  "appeals",
  {
    ...
    userId: integer("user_id"),
    grounds: text("grounds").notNull().default("Unsatisfactory resolution"),
    reason: text("reason").notNull().default(""),
    status: text("status").notNull().default("Under review"),
    stage: integer("stage").notNull().default(1),
    officer: text("officer").notNull().default("Appellate Authority"),
    officerDesignation: text("officer_designation").notNull().default("Joint Secretary / First Appellate Authority"),
    officerEmail: text("officer_email").notNull().default("appellate.authority@gov.in"),
    officerPhone: text("officer_phone").notNull().default("+91-11-2338-9900"),
    officerOffice: text("officer_office").notNull().default("Appellate Directorate, CGO Complex, New Delhi"),
    slaDeadline: text("sla_deadline").notNull().default(""),
    createdAt: text("created_at").notNull().default(nowText),
  },
